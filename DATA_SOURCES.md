...
10. **EA surface water** — WMS
    ...
    covered (`EA_COVERAGE_MIN`, alpha ≥ 64), fitted against the EA's own
    residential counts and held out (1.17× / 1.02× the EA level on the
    held-out tiles, 1.18× / 1.00× built nationally; the old alpha > 16 read
    1.60× / 1.53×). The depth layers use the same rule.
11. **NRW surface water** — same GeoServer, layer
    ...
    16 coverage levels (full pixel = alpha 102); read at alpha > 16. The
    EA's 25% rule was built and measured here and NOT adopted: NRW's own
    people-at-risk counts put Welsh sw_high at ~0.56× their level already,
    and 25% made it ~0.43× and Spearman +0.72 → +0.69. FRAW SW is notably more
